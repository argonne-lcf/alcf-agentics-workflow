...
Can be run as a standalone script:
   python sim_kernel.py params.json

The JSON file should contain: protein, timestep, temperature, steps.
Results are printed to stdout as a single JSON line.
"""
...
if __name__ == "__main__":
   if len(sys.argv) != 2:
      print(f"Usage: python {sys.argv[0]} <params.json>", file=sys.stderr)
      sys.exit(1)

   params_path = Path(sys.argv[1])
   if not params_path.exists():
      print(f"Error: file not found: {params_path}", file=sys.stderr)
      sys.exit(1)
