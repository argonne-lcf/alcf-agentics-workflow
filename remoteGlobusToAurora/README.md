...
globus-compute-endpoint configure --template-config my-endpoint-config.yaml my-aurora-endpoint
...
- **Usage**: `python scripts/gen_endpoint_config.py --repo-path $PWD --venv-path $PWD/venv -o endpoint-config.yaml`
